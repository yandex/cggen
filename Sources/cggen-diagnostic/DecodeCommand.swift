...
struct DecodeCommand: ParsableCommand {
  ...
  mutating func run() throws {
    ...
    let bytecodeArrays = try extractBytecode(
      from: content,
  ...
  private func extractBytecode(
    from content: String,
    fileType: String
  ) throws -> [BytecodeArray] {
    if fileType == "m" {
  ...
  private func parseObjectiveCFile(_ content: String) throws
    -> [BytecodeArray] {
    ...
    for match in matches {
      ...
      if isCompressed {
        ...
        if let sizeMatch = sizeRegex.firstMatch(
          in: content,
          options: [],
          range: NSRange(content.startIndex..., in: content)
        ),
  ...
  private func decodeCommands(_ data: [UInt8]) throws {
    try data.withUnsafeBytes { bytes in
      ...
      try BytecodeVisitor.visit(
        &bytecode,
        onSaveGState: { _, context in
          printCmd("SaveGState()", context: context)
        },
        onRestoreGState: { _, context in printCmd(
          "RestoreGState()",
          context: context
        ) },
        onMoveTo: { point, context in printCmd(
          "MoveTo(\(formatFloat(point.x)), \(formatFloat(point.y)))",
          context: context
        ) },
        onLineTo: { point, context in printCmd(
          "LineTo(\(formatFloat(point.x)), \(formatFloat(point.y)))",
          context: context
        ) },
        onCurveTo: { curve, context in
          printCmd(
            "CurveTo(\(formatFloat(curve.to.x)), \(formatFloat(curve.to.y)), \(formatFloat(curve.control1.x)), \(formatFloat(curve.control1.y)), \(formatFloat(curve.control2.x)), \(formatFloat(curve.control2.y)))",
            context: context
          )
        },
        onQuadCurveTo: { curve, context in
          printCmd(
            "QuadCurveTo(\(formatFloat(curve.to.x)), \(formatFloat(curve.to.y)), \(formatFloat(curve.control.x)), \(formatFloat(curve.control.y)))",
            context: context
          )
        },
        onClosePath: { _, context in
          printCmd("ClosePath()", context: context)
        },
        onAppendRectangle: { rect, context in
          printCmd(
            "AppendRectangle(\(formatFloat(rect.origin.x)), \(formatFloat(rect.origin.y)), \(formatFloat(rect.size.width)), \(formatFloat(rect.size.height)))",
            context: context
          )
        },
        onAppendRoundedRect: { args, context in
          let (rect, rx, ry) = args
          printCmd(
            "AppendRoundedRect(\(formatFloat(rect.origin.x)), \(formatFloat(rect.origin.y)), \(formatFloat(rect.size.width)), \(formatFloat(rect.size.height)), \(formatFloat(rx)), \(formatFloat(ry)))",
            context: context
          )
        },
        onAddEllipse: { rect, context in
          printCmd(
            "AddEllipse(\(formatFloat(rect.origin.x)), \(formatFloat(rect.origin.y)), \(formatFloat(rect.size.width)), \(formatFloat(rect.size.height)))",
            context: context
          )
        },
        onAddArc: { args, context in
          ...
          printCmd(
            "AddArc(\(formatFloat(center.x)), \(formatFloat(center.y)), \(formatFloat(radius)), \(formatFloat(startAngle)), \(formatFloat(endAngle)), \(clockwise ? 1 : 0))",
            context: context
          )
        },
        ...
        onFillWithRule: { rule, context in
          printCmd(
            "FillWithRule(\(rule == .winding ? "winding" : "evenOdd"))",
            context: context
          )
        },
        onStroke: { _, context in printCmd("Stroke()", context: context) },
        onFillAndStroke: { _, context in printCmd(
          "FillAndStroke()",
          context: context
        ) },
        ...
        onStrokeColor: { color, context in
          printCmd(
            "StrokeColor(\(color.red), \(color.green), \(color.blue))",
            context: context
          )
        },
        onFillAlpha: { alpha, context in printCmd(
          "FillAlpha(\(formatFloat(Float32(alpha))))",
          context: context
        ) },
        onStrokeAlpha: { alpha, context in printCmd(
          "StrokeAlpha(\(formatFloat(Float32(alpha))))",
          context: context
        ) },
        onFillNone: { _, context in printCmd("FillNone()", context: context) },
        onStrokeNone: { _, context in
          printCmd("StrokeNone()", context: context)
        },
        onLineWidth: { width, context in printCmd(
          "LineWidth(\(formatFloat(Float32(width))))",
          context: context
        ) },
        ...
        onMiterLimit: { limit, context in printCmd(
          "MiterLimit(\(formatFloat(Float32(limit))))",
          context: context
        ) },
        onDash: { pattern, context in
          let lengths = pattern.lengths.map { formatFloat(Float32($0)) }
            .joined(separator: ", ")
          printCmd(
            "Dash(\(formatFloat(Float32(pattern.phase))), [\(lengths)])",
            context: context
          )
        },
        onDashPhase: { phase, context in printCmd(
          "DashPhase(\(formatFloat(Float32(phase))))",
          context: context
        ) },
        onDashLengths: { lengths, context in
          let lengthsStr = lengths.map { formatFloat(Float32($0)) }
            .joined(separator: ", ")
          printCmd("DashLengths([\(lengthsStr)])", context: context)
        },
        onConcatCTM: { transform, context in
          printCmd(
            "ConcatCTM(\(formatFloat(Float32(transform.a))), \(formatFloat(Float32(transform.b))), \(formatFloat(Float32(transform.c))), \(formatFloat(Float32(transform.d))), \(formatFloat(Float32(transform.tx))), \(formatFloat(Float32(transform.ty))))",
            context: context
          )
        },
        onGlobalAlpha: { alpha, context in printCmd(
          "GlobalAlpha(\(formatFloat(Float32(alpha))))",
          context: context
        ) },
        onSetGlobalAlphaToFillAlpha: { _, context in printCmd(
        ...
        onFillLinearGradient: { args, context in
          let (id, options) = args
          printCmd(
            "FillLinearGradient(\(id), \(formatFloat(options.start.x)), \(formatFloat(options.start.y)), \(formatFloat(options.end.x)), \(formatFloat(options.end.y)))",
            context: context
          )
        },
        onFillRadialGradient: { args, context in
          let (id, options) = args
          printCmd(
            "FillRadialGradient(\(id), \(formatFloat(options.startCenter.x)), \(formatFloat(options.startCenter.y)), \(formatFloat(Float32(options.startRadius))), \(formatFloat(options.endCenter.x)), \(formatFloat(options.endCenter.y)), \(formatFloat(Float32(options.endRadius))))",
            context: context
          )
        },
        onStrokeLinearGradient: { args, context in
          let (id, options) = args
          printCmd(
            "StrokeLinearGradient(\(id), \(formatFloat(options.start.x)), \(formatFloat(options.start.y)), \(formatFloat(options.end.x)), \(formatFloat(options.end.y)))",
            context: context
          )
        },
        onStrokeRadialGradient: { args, context in
          let (id, options) = args
          printCmd(
            "StrokeRadialGradient(\(id), \(formatFloat(options.startCenter.x)), \(formatFloat(options.startCenter.y)), \(formatFloat(Float32(options.startRadius))), \(formatFloat(options.endCenter.x)), \(formatFloat(options.endCenter.y)), \(formatFloat(Float32(options.endRadius))))",
            context: context
          )
        },
        onLinearGradient: { args, context in
          let (id, options) = args
          printCmd(
            "LinearGradient(\(id), \(formatFloat(options.start.x)), \(formatFloat(options.start.y)), \(formatFloat(options.end.x)), \(formatFloat(options.end.y)))",
            context: context
          )
        },
        onRadialGradient: { args, context in
          let (id, options) = args
          printCmd(
            "RadialGradient(\(id), \(formatFloat(options.startCenter.x)), \(formatFloat(options.startCenter.y)), \(formatFloat(Float32(options.startRadius))), \(formatFloat(options.endCenter.x)), \(formatFloat(options.endCenter.y)), \(formatFloat(Float32(options.endRadius))))",
            context: context
          )
        },
        ...
        onClipWithRule: { rule, context in
          printCmd(
            "ClipWithRule(\(rule == .winding ? "winding" : "evenOdd"))",
            context: context
          )
        },
        onClipToRect: { rect, context in
          printCmd(
            "ClipToRect(\(formatFloat(rect.origin.x)), \(formatFloat(rect.origin.y)), \(formatFloat(rect.size.width)), \(formatFloat(rect.size.height)))",
            context: context
          )
        },
        onBeginTransparencyLayer: { _, context in printCmd(
          "BeginTransparencyLayer()",
          context: context
        ) },
        onEndTransparencyLayer: { _, context in printCmd(
          "EndTransparencyLayer()",
          context: context
        ) },
        onShadow: { shadow, context in
          printCmd(
            "Shadow(\(formatFloat(shadow.offset.width)), \(formatFloat(shadow.offset.height)), \(formatFloat(Float32(shadow.blur))), \(shadow.color.red), \(shadow.color.green), \(shadow.color.blue), \(formatFloat(Float32(shadow.color.alpha))))",
            context: context
          )
        },
        onSubrouteWithId: { id, context in printCmd(
          "SubrouteWithId(\(id))",
          context: context
        ) },
        onFlatness: { flatness, context in printCmd(
          "Flatness(\(formatFloat(Float32(flatness))))",
          context: context
        ) },
        onFillEllipse: { rect, context in
          printCmd(
            "FillEllipse(\(formatFloat(rect.origin.x)), \(formatFloat(rect.origin.y)), \(formatFloat(rect.size.width)), \(formatFloat(rect.size.height)))",
            context: context
          )
        },
        ...
        onFillRule: { rule, context in
          printCmd(
            "FillRule(\(rule == .winding ? "winding" : "evenOdd"))",
            context: context
          )
        },
        onReplacePathWithStrokePath: { _, context in printCmd(
          "ReplacePathWithStrokePath()",
          context: context
        ) },
        onLines: { points, context in
          let pointsStr = points
            .map { "\(formatFloat($0.x)), \(formatFloat($0.y))" }
            .joined(separator: ", ")
  ...
  private func decodeSegment(
    _ fullBytecode: [UInt8],
    startIndex: Int,
    endIndex: Int
  ) throws {
    // Create a slice of bytecode for this segment
    guard startIndex < fullBytecode.count, endIndex <= fullBytecode.count,
          startIndex < endIndex else {
      print(
        "Invalid segment range: [\(startIndex)...\(endIndex)] for bytecode of size \(fullBytecode.count)"
      )
      return
    }
